<div class="thumbnail-movie thumbnail-serial mx-3 card {{isset($class) ? $class : ''}}" style="max-width: 220px;">
    <div class="thumb-wrapper">
        <a class="thumb" href="{{route('ShowItem',['content'=>$movie->categories->name,'slug'=>$movie->slug])}}">
            <div class="abs-fit">
                @if ($movie->picture)
                <img src="{{asset("$movie->picture")}}" alt="{{$movie->title}}" aria-label="{{$movie->title}}"
                    class="thumb-image">
                @else
                <div class="d-flex justify-content-center align-items-center h-100">
                    <img style="object-fit: cover;" src="{{asset("assets/images/logo-video1.png")}}"
                        alt="{{$movie->title}}" aria-label="{{$movie->title}}" class="thumb-image">
                </div>
                @endif
            </div>
            <div class="tools">
                <span class="badge-rate">
                    ...
                    <svg class="icon icon-thumb-up d-in v-m g-20 fs-1-2 ml-xxs" viewBox="0 0 24 24" viewBox="viewBox=" 0
                        0 24 24"">
    ...
    <div class="position-relative px-2 pt-3">
        <a href="{{route('ShowItem',['content'=>$movie->categories->name,'slug'=>$movie->slug])}}"
            title="{{$movie->title}}" class="title title d-block mb-2  "><span
                class="d-block {{preg_match('/^[^\x{600}-\x{6FF}]+$/u', $movie->title) ? 'text-left dir-ltr' : '' }}">{{Illuminate\Support\Str::limit($movie->title,22)}}</span>
